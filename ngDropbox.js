		...
		this.$get = ['$http','$log','$q','$resource','$window', function($http,$log,$q,$resource,$window){
			...
			return {
				...
				authorize_v2: function(){
					...
					var authUrl = 'https://www.dropbox.com/1/oauth2/authorize?client_id='+ settings.app_key +'&response_type=token&redirect_uri=' + settings.redirectUri
					function listener(event) {
						if (event.data){
				...
				search: function(path, query){
					return $http.post('https://api.dropbox.com/1/search/auto/'+path, {'query': query, 'include_deleted': false}, {'headers': authorizationHeader()});
				},
